...
INFERENCE_YAML = REPO_ROOT / "inference" / "config" / "inference.yaml"
...
def _resolve_inference_dir(key: str, default: str) -> Path:
    if not INFERENCE_YAML.is_file():
        raise FileNotFoundError(f"Missing inference config: {INFERENCE_YAML}")
