...
base_model = "resources/models/Z-Image-Turbo"
import os
import argparse
...
def main(
    ...
    # load transformer
    if model is None:
        raise ValueError("--model 必须提供：可以是单个 transformer 权重文件(.safetensors/.ckpt)或一个 diffusers 目录/Hub repo")

    # `from_pretrained` 只支持目录/Hub repo；单文件权重请用 `from_single_file`
    if os.path.isfile(model):
        transformer = ZImageTransformer2DModel.from_single_file(
            model,
            # 需要从一个 diffusers 模型目录读取结构配置（例如 base_model/transformer/config.json）
            config=base_model,
    ...
    image.save("/home/tonera/website/output/test_z_image_singlefile.png")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="FLUX.2-klein AWQ 文本编码器最小推理脚本")
    parser.add_argument(
        "--base-model",
        type=str,
        default=base_model,
